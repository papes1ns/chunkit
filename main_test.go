...
func createEvent(start time.Time, end time.Time, summary string, responseStatus string, self bool) *calendar.Event {
...
func Test_makeChunks(t *testing.T) {
	...
	declinedEvent := createEvent(date.Add(10*time.Hour), date.Add(12*time.Hour), "declined event", "declined", true)
	acceptedEvent := createEvent(date.Add(10*time.Hour), date.Add(12*time.Hour), "accepted event", "accepted", true)
	gapEvent := createEvent(date.Add(13*time.Hour), date.Add(14*time.Hour), "gap event", "accepted", true)
	overlapEvent := createEvent(date.Add(8*time.Hour), date.Add(17*time.Hour), "overlapping event", "accepted", true)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			chunks := makeChunks(date, test.items)
...
func Benchmark_makeChunks(b *testing.B) {
	date := time.Now()
	date = time.Date(date.Year(), date.Month(), date.Day(), 0, 0, 0, 0, date.Location())
